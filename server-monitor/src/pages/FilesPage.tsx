...
export default function FilesPage() {
  ...
  const pullRefreshStartRef = useRef<{ pane: 'left' | 'right'; y: number; ready: boolean } | null>(null)
  ...
  const editorLineNumbersRef = useRef<HTMLPreElement>(null)
  ...
  const [pullRefreshPane, setPullRefreshPane] = useState<'left' | 'right' | null>(null)
  ...
  const [bookmarkGroupAction, setBookmarkGroupAction] = useState<string | null>(null)
  const [bookmarkGroupRename, setBookmarkGroupRename] = useState<string | null>(null)
  ...
  const drawerServers = servers.filter(server => {
    ...
    const matchesQuery = !query || `${server.name} ${server.host} ${server.username}`.toLowerCase().includes(query)
    const online = isConnected(server.id)
    const matchesStatus = serverFilter === 'all' || (serverFilter === 'online' ? online : !online)
  ...
  function refreshPane(pane: 'left' | 'right') {
    const { serverId, state, setState } = getPaneInfo(pane)
    if (serverId) loadFiles(serverId, state.currentPath, setState)
  }

  function handlePanelTouchStart(pane: 'left' | 'right', event: React.TouchEvent<HTMLDivElement>) {
    if (event.currentTarget.scrollTop <= 0) pullRefreshStartRef.current = { pane, y: event.touches[0]?.clientY || 0, ready: false }
  }

  function handlePanelTouchMove(pane: 'left' | 'right', event: React.TouchEvent<HTMLDivElement>) {
    const start = pullRefreshStartRef.current
    if (!start || start.pane !== pane) return
    const distance = (event.touches[0]?.clientY || 0) - start.y
    start.ready = distance > 56
    if (start.ready) setPullRefreshPane(pane)
    else if (pullRefreshPane === pane) setPullRefreshPane(null)
  }

  function handlePanelTouchEnd(pane: 'left' | 'right') {
    if (pullRefreshStartRef.current?.pane === pane && pullRefreshStartRef.current.ready) refreshPane(pane)
    pullRefreshStartRef.current = null
    setPullRefreshPane(null)
  }
  ...
  const navigateTo = useCallback((path: string, pane: 'left' | 'right', recordHistory = true) => {
    ...
    localStorage.setItem('server-monitor-file-recents', JSON.stringify([{ serverId, path, name }, ...recents.filter(item => item.serverId !== serverId || item.path !== path)].slice(0, 30)))
  ...
  const recentPaths = (() => {
    void bookmarkVersion
    try { return JSON.parse(localStorage.getItem('server-monitor-file-recents') || '[]') as { serverId: string; path: string; name: string }[] } catch { return [] }
  })()
    ...
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-white text-[#1f2632]">
      <header className="flex h-[calc(64px+env(safe-area-inset-top))] shrink-0 items-center gap-3 bg-[#e9eef6] px-4 pt-safe">
        <button
          type="button"
          aria-label="打开服务器列表"
          onClick={() => { setServerSearch(''); setServerFilter('all'); setServerDrawerOpen(true) }}
      ...
      <main className="flex min-h-0 flex-1 bg-white">
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto px-2 pb-3 pt-1" onTouchStart={event => handlePanelTouchStart('left', event)} onTouchMove={event => handlePanelTouchMove('left', event)} onTouchEnd={() => handlePanelTouchEnd('left')}>
            {pullRefreshPane === 'left' && <div className="flex h-7 items-center justify-center text-[11px] text-[#5b8fe8]">松开刷新</div>}
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto px-2 pb-3 pt-1" onTouchStart={event => handlePanelTouchStart('right', event)} onTouchMove={event => handlePanelTouchMove('right', event)} onTouchEnd={() => handlePanelTouchEnd('right')}>
            {pullRefreshPane === 'right' && <div className="flex h-7 items-center justify-center text-[11px] text-[#5b8fe8]">松开刷新</div>}
            {!rightServerId ? (
              <div className="flex h-[28%] min-h-[130px] items-center justify-center">
                <p className="text-[20px] font-medium text-[#8e97a8]">目录为空</p>
              </div>
      ...
      <footer className="flex h-16 shrink-0 items-center justify-between border-t border-[#e6e9f0] bg-white px-7">
        <button onClick={() => navigateForward(activePane)}
          disabled={!(forwardHistory[activePane]?.length > 0)}
          className="flex h-9 w-9 items-center justify-center rounded-lg text-[#5b8fe8] transition-transform duration-150 active:scale-90 active:bg-[#eef4ff] disabled:text-[#b6c1d1] disabled:opacity-80">
        ...
        <button onClick={() => { setNewFileDialogOpen(true); setNewFileName(''); setNewFileType('file') }}
          className="flex h-9 w-9 items-center justify-center rounded-lg text-[#5b8fe8] transition-transform duration-150 active:scale-90 active:bg-[#eef4ff]">
          <Plus size={26} strokeWidth={2.2} />
        </button>
        <button onClick={syncActivePaneToOtherPane}
          disabled={activePane === 'left' ? !activeTab : !rightServerId}
          aria-label="同步当前目录到另一面板"
          className="flex h-9 w-9 items-center justify-center gap-0.5 rounded-lg transition-transform duration-150 active:scale-90 active:bg-[#eef4ff] disabled:opacity-40">
          <ArrowLeft size={16} strokeWidth={2.4} className={activePane === 'right' ? 'text-[#5b8fe8]' : 'text-[#b6c1d1]'} />
          <ArrowRight size={16} strokeWidth={2.4} className={activePane === 'left' ? 'text-[#5b8fe8]' : 'text-[#b6c1d1]'} />
        </button>
          ...
          <aside className="absolute inset-y-0 left-0 flex w-[84vw] max-w-[360px] flex-col bg-white shadow-2xl animate-slide-right">
            ...
            <div className="shrink-0 px-4 py-4">
              <label className="flex h-11 items-center gap-2 rounded-xl bg-[#f4f6fa] px-3 text-[#7d8798]">
                <Search size={18} />
                <input value={serverSearch} onChange={event => setServerSearch(event.target.value)} placeholder="搜索服务器" className="min-w-0 flex-1 bg-transparent text-sm text-[#1f2632] outline-none placeholder:text-[#9ca5b5]" />
                {serverSearch && <button aria-label="清除搜索" onClick={() => setServerSearch('')}><X size={16} /></button>}
              </label>
              <div className="mt-3 flex gap-2">
                {([['all', '全部'], ['online', '在线'], ['offline', '离线']] as const).map(([value, label]) => (
                  <button key={value} type="button" onClick={() => setServerFilter(value)} className={`rounded-lg px-3 py-1.5 text-xs font-medium transition-colors ${serverFilter === value ? 'bg-[#5b8fe8] text-white' : 'bg-[#f1f4f8] text-[#748096]'}`}>{label}</button>
                ))}
              </div>
            </div>
      ...
      {/* 文本编辑器 */}
      {editingFile && (
          <div className="fixed inset-0 z-[2000] flex flex-col bg-white pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)]">
          ...
          {editorSearchOpen && <div className="flex items-center gap-2 border-b border-gray-100 px-3 py-2"><input ref={editorSearchInputRef} value={editorSearchQuery} onChange={e => { setEditorSearchQuery(e.target.value); setEditorSearchIndex(0) }} onKeyDown={e => { if (e.key === 'Enter') navigateEditorSearch(e.shiftKey ? -1 : 1) }} placeholder="查询文本" className="min-w-0 flex-1 rounded-lg bg-gray-100 px-3 py-2 text-sm outline-none" /><span className="text-xs text-gray-500">{editorMatches.length ? `${editorSearchIndex + 1}/${editorMatches.length}` : '0/0'}</span><button onClick={() => navigateEditorSearch(-1)} aria-label="上一个查询结果" className="p-1 text-gray-600"><ChevronLeft size={18} /></button><button onClick={() => navigateEditorSearch(1)} aria-label="下一个查询结果" className="p-1 text-gray-600"><ChevronRight size={18} /></button></div>}
          <div className="flex min-h-0 flex-1 overflow-hidden bg-gray-50">
            <div className="h-full shrink-0 overflow-hidden border-r border-gray-200 bg-[#f4f5f7]"><pre ref={editorLineNumbersRef} aria-hidden="true" className="select-none px-2 py-3 text-right font-mono text-sm leading-6 text-gray-400 will-change-transform">{Array.from({ length: Math.max(1, editingFile.content.split('\n').length) }, (_, index) => index + 1).join('\n')}</pre></div>
            <textarea ref={editorTextareaRef} value={editingFile.content} onScroll={event => { if (editorLineNumbersRef.current) editorLineNumbersRef.current.style.transform = `translateY(-${event.currentTarget.scrollTop}px)` }} onKeyDown={event => { if (event.key === 'Tab') { event.preventDefault(); const target = event.currentTarget; const start = target.selectionStart; setEditingFile((prev: any) => prev ? { ...prev, content: prev.content.slice(0, start) + '  ' + prev.content.slice(target.selectionEnd) } : null); requestAnimationFrame(() => target.setSelectionRange(start + 2, start + 2)) } }} onChange={e => setEditingFile((prev: typeof editingFile) => prev ? { ...prev, content: e.target.value } : null)} className="h-full min-w-0 flex-1 resize-none overflow-auto bg-transparent px-3 py-3 font-mono text-sm leading-6 text-gray-800 outline-none" spellCheck={false} readOnly={editingFile.isTruncated} />
          </div>
          ...
          <div className="flex items-center justify-center gap-3 border-t border-gray-100 px-4 py-2"><button onClick={() => _loadEditorChunk(Math.max(0, (editingFile.chunkOffset || 0) - LARGE_FILE_CHUNK_BYTES))} disabled={!editingFile.isTruncated || !editingFile.chunkOffset} className="rounded-lg bg-gray-100 px-3 py-1.5 text-xs text-gray-600 disabled:opacity-40">上一页</button><span className="text-xs text-gray-400">{editingFile.isTruncated ? `第 ${Math.floor((editingFile.chunkOffset || 0) / LARGE_FILE_CHUNK_BYTES) + 1} 页` : '单页文件'}</span><button onClick={() => _loadEditorChunk((editingFile.chunkOffset || 0) + LARGE_FILE_CHUNK_BYTES)} disabled={!editingFile.isTruncated || (editingFile.chunkOffset || 0) + LARGE_FILE_CHUNK_BYTES >= editingFile.totalSize} className="rounded-lg bg-gray-100 px-3 py-1.5 text-xs text-gray-600 disabled:opacity-40">下一页</button></div>
        </div>
      )}
      ...
      {bookmarksDrawerOpen && (
        <div className="fixed inset-0 z-[2100] flex flex-col justify-end">
          ...
          <section className={`relative flex w-full flex-col overflow-hidden rounded-t-2xl border-t border-[#e4e7ed] bg-white shadow-2xl animate-slide-up transition-[height] duration-200 ${bookmarksHeight === 'full' ? 'h-[92vh]' : 'h-[58vh]'}`}>
            ...
            <header className="relative flex shrink-0 items-end border-b border-[#e8ebf0] px-5 pt-3">
              <div className="flex min-w-0 flex-1 items-end gap-8 overflow-x-auto">
                {[['recent', '最近'], ['bookmarks', '书签'], ...bookmarkGroups.map(group => [group, group])].map(([tab, label]) => (
                  <button key={tab} onClick={() => setBookmarkDrawerTab(tab)} onContextMenu={event => { if (tab !== 'recent' && tab !== 'bookmarks') { event.preventDefault(); setBookmarkGroupAction(tab) } }} onTouchStart={() => { if (tab !== 'recent' && tab !== 'bookmarks') longPressTimerRef.current = window.setTimeout(() => setBookmarkGroupAction(tab), 450) }} onTouchEnd={() => { if (longPressTimerRef.current) clearTimeout(longPressTimerRef.current) }} className={`relative shrink-0 pb-3 text-[17px] ${bookmarkDrawerTab === tab ? 'font-semibold text-[#171b22]' : 'font-medium text-[#7c8490]'}`}>{label}{bookmarkDrawerTab === tab && <span className="absolute -bottom-px left-1/2 h-[3px] w-20 -translate-x-1/2 rounded-full bg-[#5b9bff]" />}</button>
                ))}
              </div>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-3">
              ...
              {(bookmarkDrawerTab === 'recent' ? recentPaths : savedBookmarks.filter(bookmark => bookmarkDrawerTab === 'bookmarks' ? !bookmark.group : bookmark.group === bookmarkDrawerTab)).length === 0 && <p className="pt-16 text-center text-sm text-[#8992a0]">{bookmarkDrawerTab === 'recent' ? '暂无最近记录' : '暂无书签'}</p>}
            </div>
          </section>
        </div>
      )}
      ...
      {bookmarkGroupDialogOpen && (
        <div className="fixed inset-0 z-[2200] flex items-center justify-center p-5">
          ...
          <div className="relative w-full max-w-xs rounded-2xl bg-white p-5 shadow-xl animate-scale-in">
            ...
            <div className="flex gap-2"><button onClick={() => setBookmarkGroupDialogOpen(false)} className="flex-1 rounded-xl bg-gray-100 py-2.5 text-sm text-gray-600">取消</button><button onClick={() => { const name = bookmarkGroupName.trim(); if (!name || bookmarkGroups.includes(name)) return; localStorage.setItem('server-monitor-file-bookmark-groups', JSON.stringify([...bookmarkGroups, name])); setBookmarkDrawerTab(name); setBookmarkVersion(version => version + 1); setBookmarkGroupDialogOpen(false) }} className="flex-1 rounded-xl bg-blue-50 py-2.5 text-sm text-blue-600">确定</button></div>
          </div>
        </div>
      )}
      ...
      {bookmarkGroupAction && (
        <div className="fixed inset-0 z-[2200] flex items-center justify-center p-5">
          <div className="absolute inset-0 bg-black/30" onClick={() => setBookmarkGroupAction(null)} />
          <div className="relative w-full max-w-[330px] overflow-hidden rounded-md bg-white py-1 shadow-xl animate-scale-in">
            {(() => { const index = bookmarkGroups.indexOf(bookmarkGroupAction); return <>
              <button disabled={index === 0} onClick={() => { const groups = [...bookmarkGroups]; [groups[index - 1], groups[index]] = [groups[index], groups[index - 1]]; localStorage.setItem('server-monitor-file-bookmark-groups', JSON.stringify(groups)); setBookmarkVersion(version => version + 1); setBookmarkGroupAction(null) }} className="w-full px-6 py-4 text-left text-[16px] text-[#181c22] disabled:text-[#c5cad3]">← 左移</button>
              <button disabled={index === bookmarkGroups.length - 1} onClick={() => { const groups = [...bookmarkGroups]; [groups[index], groups[index + 1]] = [groups[index + 1], groups[index]]; localStorage.setItem('server-monitor-file-bookmark-groups', JSON.stringify(groups)); setBookmarkVersion(version => version + 1); setBookmarkGroupAction(null) }} className="w-full px-6 py-4 text-left text-[16px] text-[#181c22] disabled:text-[#c5cad3]">右移 -&gt;</button>
              <button onClick={() => { setBookmarkGroupRename(bookmarkGroupAction); setBookmarkGroupName(bookmarkGroupAction); setBookmarkGroupAction(null) }} className="w-full px-6 py-4 text-left text-[16px] text-[#181c22]">重命名</button>
              <button onClick={() => { const groups = bookmarkGroups.filter(group => group !== bookmarkGroupAction); const bookmarks = savedBookmarks.map(bookmark => bookmark.group === bookmarkGroupAction ? { ...bookmark, group: undefined } : bookmark); localStorage.setItem('server-monitor-file-bookmark-groups', JSON.stringify(groups)); localStorage.setItem('server-monitor-file-bookmarks', JSON.stringify(bookmarks)); setBookmarkDrawerTab('bookmarks'); setBookmarkVersion(version => version + 1); setBookmarkGroupAction(null) }} className="w-full px-6 py-4 text-left text-[16px] text-[#181c22]">删除</button>
            </> })()}
          </div>
        </div>
      )}
      ...
      {bookmarkGroupRename && (
        <div className="fixed inset-0 z-[2300] flex items-center justify-center p-5">
          <div className="absolute inset-0 bg-black/30" onClick={() => setBookmarkGroupRename(null)} />
          <div className="relative w-full max-w-xs rounded-2xl bg-white p-5 shadow-xl animate-scale-in">
            <h3 className="mb-3 text-sm font-semibold text-gray-800">重命名分组</h3>
            <input value={bookmarkGroupName} onChange={event => setBookmarkGroupName(event.target.value)} autoFocus className="mb-4 w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm text-gray-800 outline-none focus:border-blue-300" />
            <div className="flex gap-2"><button onClick={() => setBookmarkGroupRename(null)} className="flex-1 rounded-xl bg-gray-100 py-2.5 text-sm text-gray-600">取消</button><button onClick={() => { const name = bookmarkGroupName.trim(); if (!name || bookmarkGroups.includes(name)) return; const groups = bookmarkGroups.map(group => group === bookmarkGroupRename ? name : group); const bookmarks = savedBookmarks.map(bookmark => bookmark.group === bookmarkGroupRename ? { ...bookmark, group: name } : bookmark); localStorage.setItem('server-monitor-file-bookmark-groups', JSON.stringify(groups)); localStorage.setItem('server-monitor-file-bookmarks', JSON.stringify(bookmarks)); setBookmarkDrawerTab(name); setBookmarkVersion(version => version + 1); setBookmarkGroupRename(null) }} className="flex-1 rounded-xl bg-blue-50 py-2.5 text-sm text-blue-600">确定</button></div>
          </div>
        </div>
      )}
